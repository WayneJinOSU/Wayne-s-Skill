...
11. 若出现在 final_report 中形成估值结论，必须删除；若只是市场隐含预期、最小利润模型、PEG 估值因子、情景准入、年份切换或质量折价，可移至正文利润模型或 post-report `peg_valuation_handoff` 的待承接清单；若是三表/FCF 驱动、PEG-ready 候选字段、DCF-ready 候选字段、WACC、UFCF 或 validation 驱动，可移至 post-report `dcf_financial_model_handoff` 的待承接清单，但不得形成目标价、目标市值、买卖建议或“当前贵不贵”。
