...
1. `$akshare` 盈利预测主接口：如 `stock_profit_forecast_ths`。
2. `$akshare` 盈利预测备用接口：如 `stock_profit_forecast_em` 全量过滤绕路。
3. `$wencai-query` / pywencai：分年查询，不用宽泛长句作为唯一判断。

只要任一来源取得“预测年份 + 机构数 + 净利润或 EPS 均值”，即先标记为 `consensus_available` 或 `partial_consensus_available`，再做口径校验；不要直接写 `data_gap`。

禁止事项：

- 不使用非接口页面、网页截取、公开搜索或手工摘录作为一致预期来源。
- 不通过浏览器页面反编译字段；接口失败就换下一个 skill/API 接口，全部失败才记 `data_gap`。
- 不把单家预测、社区预测或业绩预告冒充一致预期。
...
该接口能同时取得净利润、EPS、机构数时，默认可作为 A 股 PEG 一致预期主源。这里的“同花顺”只指 AkShare 接口名和返回口径，不允许手工读取网页。
...
可用字段通常包括研报数、未来年度 EPS、近六月买入/增持/中性/减持/卖出数量。若只有 EPS 没有净利润，可作为主接口/Wencai 的交叉验证；若需从 EPS 推净利润，必须用同一日期股本并标注为“推算值”。
