...
- 估值输入以 `<标的>_peg_ready_package.md` 为主数据源；`<标的>_peg_valuation_handoff.md` 只提供 PEG 档位、质量折价和年份切换规则。
...
```text
<标的>_question.md
<标的>_peg_ready_package.md 或 <标的>_financial_model_output.md
...
- 若只有 `<标的>_peg_valuation_handoff.md`，但没有 `$financial-modeling` 输出的 `<标的>_peg_ready_package.md`，不得输出正式目标市值、目标价或最终 PEG 结论；只能输出“估值准备清单/缺口清单”。
- `<标的>_peg_valuation_handoff.md` 中出现的利润锚只能视为投研候选锚或情景边界，不能视为已完成口径统一、YoY/CAGR 校验和现金流质量校验的 PEG-ready 数据包。
- `<标的>_financial_model_output.md` 只有在明确包含 PEG-ready Package 最低字段时，才可替代 `<标的>_peg_ready_package.md`；否则仍视为建模缺口。

必须额外获取并注明日期和来源：

- 最新股价、总市值、股本、TTM PE、静态 PE、PB。
- 市场一致预期：至少覆盖未来两年；能取得时覆盖第三年。
- PEG-ready 数据包：优先来自 `$financial-modeling` 输出，包含扣非利润/经营利润、YoY、2-3 年 CAGR、一致预期对照、股本/市值、质量说明。若尚未建模，必须说明缺口，不得让 `peg_valuation_handoff` 代替 PEG-ready 数据包或三表模型。
...
1. 正式估值必须同时覆盖：
   - 市场一致预期
   - 来自 `$financial-modeling` 的 PEG-ready 数据包
   - PEG-ready 数据包中的自有中性/乐观利润锚
   - 动态 PEG
   - 复合 PEG
   - 当前市值隐含利润
   - 中性与乐观目标市值
...
正式估值前优先读取 `$financial-modeling` 输出的 PEG-ready 数据包。最低字段：
...
| `own_base_profit_2026E/2027E/2028E` | 中性利润锚 |
| `own_bull_profit_2026E/2027E/2028E` | 乐观利润锚 |
...
若 PEG-ready 数据包缺失，必须停止在正式估值前，并输出：

```text
缺失的 PEG-ready 字段：
可从 peg_valuation_handoff 继承的规则：
必须由 financial-modeling 补齐的利润/YoY/CAGR/现金流质量字段：
本次不能输出的估值结论：
```
...
```text
输出范围：PEG-only / PEG + DCF待聚合 / DCF缺失需补跑
