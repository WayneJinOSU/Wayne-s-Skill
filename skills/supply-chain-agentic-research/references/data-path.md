# 数据路径与证据优先级

## 目标

本 skill 不直接替代 `$tushare`、`$wencai-query`、公告 PDF 处理、客户 capex、券商/产业链线索或海关/出货数据能力。它负责确定证据链、调用顺序、市场变量扩表和口径解释。

## 证据优先级

1. 官方公告原文：年报、季报、业绩预告/快报、订单、合同、募投、定增、可转债、问询回复、业绩说明会。
2. 客户和行业一手资料：客户财报、资本开支指引、电话会、供应链公开数据、产品平台路线、行业协会或权威机构数据。
3. `$tushare` skill：股价、市值、PE/PB、财务三表、财务指标、现金流、历史行情、行业和股本。
4. `$wencai-query` skill：题材概念、市场排名、同行筛选、阶段涨幅、事件性条件和快速交叉验证。
5. 公司官网、投资者关系、交易所互动、可靠新闻和券商纪要：用于补充客户、订单、产品、产能和产业趋势。
6. 海关、出货、排产和产业链跟踪数据：用于验证订单是否真实放量。若来源不是官方，必须说明可靠性和口径限制。

公告原文与结构化数据冲突时，以公告原文为准，并说明差异口径。行情和估值必须注明查询日期。

## Token Budget And Single-Source Rules

默认采用“单源取数，失败 fallback”，而不是多源并行交叉验证。

| 数据类型 | 默认数据源 | fallback 条件 |
| --- | --- | --- |
| 三表/财务指标 | `$tushare` 或项目既定结构化源二选一 | 接口失败、权限不可访问、关键字段缺失、字段明显异常 |
| 行情/市值/股本 | `$tushare` `daily_basic` 或等价最新市场快照 | 数据不可访问、停牌/复权口径异常、缺市值/股本 |
| 公告/年报/投关 | 巨潮/CNINFO 或交易所原文 | 原文不可访问、PDF损坏、缺附件 |
| 一致预期/研报预测 | 东方财富研报列表或项目既定一致预期源 | 近端预测缺失或字段不全 |
| 题材/市场热词 | `$wencai-query` | 只在需要市场认知差、题材扩散或行情排名时使用 |
| Gemini/Google Search | 缺口型二次线索发现 | 只在常规来源覆盖不了关键变量时运行；只进入 evidence_queue，永不直接进入 facts_core |

同一字段只允许一个事实来源进入 `facts_core`。需要复核时只记录“口径冲突/待复核”，不要把多个来源的同一字段在多个中间稿里反复铺开。

原始表读取限制：

- 不要对 `*.csv`、问财 JSON/CSV、公告索引和研报列表做宽泛 `rg`。长行命中会造成上下文爆炸。
- 读取原始 CSV 前，先用脚本、`head`、`cut`、`csvcut` 或小型解析代码抽取 10-30 行核心字段。
- 中间稿只引用抽取后的窄表和 `Fact-ID`，不要粘贴完整三表、完整研报列表或完整公告列表。
- 保存原始 CSV 可以，但默认不再读入上下文；只有字段缺失或异常排查时才打开原始表。

## 市场变量分层

供应链成长股常先由产业链和券商口径交易，后由公告和财报验证。公告确定事实底座，市场口径识别弹性变量；关键变量要进入主线、天花板和利润斜率模型，同时标明证据等级。

证据台账必须把重要变量分为四类。表格只用于索引和检查，不能替代正文推理：

| 类型 | 例子 | 写法 |
| --- | --- | --- |
| 已证实事实 | 公告订单、财报利润、公司明确披露的产品和产能 | 可进入主结论 |
| 一手行业/客户资料 | 客户 capex、终端路线图、行业协会数据、公司官网产品参数 | 可支撑产业周期和技术路线 |
| 市场/券商口径 | 券商模型假设、专家问答、产业链调研、交易热词 | 作为情景变量进入主线和敏感性 |
| 弱线索/传闻 | 无法交叉验证的份额、良率、客户认证、订单排产 | 作为跟踪变量，说明若成立/不成立的影响 |

对高重要市场变量，必须用段落写清：

- 市场正在交易什么变量。
- 为什么会带来新增需求：下游变化如何改变用量、价值量、规格、认证门槛或供需关系。
- 公司在哪个环节受益：公司是材料、零部件、设备、制造、系统、渠道，还是控股/参股公司受益。
- 财务传导路径：影响量、价、毛利率、份额、费用、折旧、良率、少数股东权益、投资收益中的哪几项。
- 证据属性：公告事实、客户/行业一手资料、市场/券商口径、情景假设或弱线索。
- 变量增强时：收入、ASP、毛利率、订单稳定性、现金流或利润中枢会怎样变化。
- 变量减弱时：逻辑会降级成什么，是周期修复、补库涨价、低端放量、扩产先行、成本挤压，还是主线证伪。

## 单公司深度研究

默认路径：

```text
1. 形成主线假设
2. 读最新年报/季报/业绩预告/重大事项公告
3. 选择一个结构化财务数据源拉财务、现金流、历史行情、市值和 PE/PB；仅在失败或缺字段时 fallback
4. 写 `<标的>_facts_core.md`，把公告事实、财务字段、分业务数据、行情估值快照统一成 Fact-ID
5. 调用 `$wencai-query` skill 查概念、同行、阶段涨幅、市场排名和交易热词；只保留窄表摘要
6. 先用普通搜索抓取券商研报、产业链调研、专家纪要、客户/竞品资料、官网参数和原材料价格线索，写入 `<标的>_evidence_queue.md`；仅在关键变量仍有缺口时，用 Gemini 做 1-2 组二次线索搜索
7. 核验关键线索后更新 `<标的>_facts_core.md` 和 `<标的>_evidence_index.md`
8. 先写 `<标的>_market_variables_map.md`，用段落写清核心变量的影响逻辑
9. 写 `<标的>_industry_cycle_supply_demand.md`，只保留能穿透到公司订单、ASP、毛利率、份额和现金流的行业变量
10. 若存在技术路线或产品代际快速变化，读取 `technology-route-value-chain.md`，写 `<标的>_architecture_value_ceiling.md` 和 `<标的>_product_generation_matrix.md`
11. 写 `<标的>_competition_customer_chain.md`，把供应链地位拆成客户链、认证阶段、份额、替代风险和竞品反证
12. 对关键原料和产能升级，分别写 `<标的>_raw_material_price_chain.md` 和 `<标的>_capacity_second_curve.md`
13. 写 `<标的>_orders_business_validation.md`，验证订单、排产、出货、收入确认、利润释放和现金回收
14. 写 `<标的>_profit_bridge.md`、`<标的>_tracking_dashboard.md` 和两类估值接力候选输入；`dcf_financial_model_handoff` 与 `peg_valuation_handoff` 只在终稿 gate PASS 后生成。后续正式估值分别交给 `$growth-stock-valuation` 和 `$dcf-valuation-workflow`
15. 写中期结构 QA、终审事实 QA、提纲、扩写蓝图和投资人可读报告，并在终稿前检查关键市场变量是否被无意删掉
```

注意：上面序号代表逻辑顺序；角色数量和模块覆盖仍按 agent-orchestration 执行，不因 token 优化而减少。

## 快速判断

当用户只要快速看一只股票，可以使用本 skill 的轻量模式，并必须说明未启用完整 agentic 流程。

轻量模式至少覆盖：

- 市场真正交易的 3 到 5 个变量。
- 技术路线或价值量变化。
- 订单出货与经营验证。
- 利润中枢和最敏感变量。

不能把轻量模式输出包装成完整深度报告。

## 横向比较或选股

当用户要求“找一批”“横向比较”“谁更强”：

```text
`$wencai-query` skill：先筛候选池、题材口径和市场排名
`$tushare` skill：补财务、估值、现金流和行情指标
公告原文：只抽 Top 候选或争议公司的关键公告
产业数据：按环节比较客户 capex、价值量、订单和竞争扩产
```

不要对所有候选公司逐一读年报，先用数据缩小范围。

## 订单/扩产/业绩驱动

当研究重点是订单、扩产、业绩预告、海外基地、客户验证：

```text
公告原文优先
订单/排产/出货/海关数据辅助
`$tushare` skill 补市值、行情、现金流和财务承受能力
`$wencai-query` skill 补市场反应和同类标的
```

必须区分“事实已公告”“公司预计”“产业链推断”“市场情绪”。

## 子模块对应数据

| 模块 | 优先数据 | 辅助数据 |
| --- | --- | --- |
| 市场变量扩表 | 券商研报、产业链调研、交易热词、官网参数、竞品文件 | wencai 市场表现、专家纪要、可靠媒体 |
| 行业周期与供需格局 | 行业协会、客户 capex、客户财报/电话会、终端平台出货、价格和库存数据 | 券商行业报告、同行公告、第三方出货数据 |
| 架构/技术路线与天花板 | 客户路线图、产品规格、终端平台、竞品资料 | 券商模型、专家纪要、公司官网参数 |
| 竞争格局与客户认证链 | 客户/竞品公告、供应商清单、客户认证、份额、竞品产能和产品代际 | 专家纪要、券商口径、产业链调研、招股书/募资书 |
| 原材料价格链 | 铜、树脂、玻纤、化工品、金属、芯片等原料价格和供需 | 同行调价、库存、存货跌价、客户价格条款 |
| 产能升级与第二曲线 | 基地产能、募投、在建工程、海外基地、控股/参股公司 | 客户导入、设备采购、人员研发、产能利用率 |
| 订单出货与经营验证 | 订单公告、量产、出货、海关、排产、客户认证、份额、替代周期 | 财报收入、毛利率、存货、应收、合同负债、现金流 |
| 利润桥 | 财报分产品、毛利率、订单、产能和成本 | 行业毛利率、同行净利率、募投效益 |
| 跟踪体系与警报信号 | 财报日历、订单/认证/投产事件、客户 capex、原材料价格、竞品认证 | 利润桥敏感变量、市场变量覆盖 QA、终审事实 QA |
| PEG 边界候选输入 | 市场阶段、主线强度、兑现阶段、利润质量、订单可见度、第二曲线、竞争壁垒、现金流质量和证伪压力 | 中期只沉淀因子、证据等级、情景边界和缺口；`peg_valuation_handoff` 在终稿 gate PASS 后输出 |
| DCF 建模候选输入 | 财报现金流、利润桥、tracking dashboard、evidence grading 中的三表/FCF 候选字段 | Revenue、EBIT、Tax、D&A、Capex、ΔNWC、UFCF、现金流质量和阻断缺口；`dcf_financial_model_handoff` 在终稿 gate PASS 后输出，且只写准入和 guardrails |

## 关键数据口径

必须注明：

- 股价、市值和估值的查询日期。
- 财报或业绩预告的报告期和披露日。
- 客户 capex 的来源、币种、会计年度和是否为公司相关业务。
- 订单金额是否含税、是否框架协议、是否已量产或收入确认。
- 海关/出货数据的统计口径、商品编码、地区和时间范围。
- 利润假设是事实、合理推断还是情景假设。

## 证据强度分级

| 证据等级 | 例子 | 用法 |
| --- | --- | --- |
| 强证据 | 公告订单、财报利润、客户 capex 指引、明确量产出货 | 可支撑主结论 |
| 中证据 | 扩产、设备、认证、客户合作、行业出货趋势 | 可支撑推断，并需要订单、财报或现金流验证 |
| 弱证据 | 传闻、概念、未量化表述、笼统进入供应链 | 只能作为线索 |

对“高重要、低证据”的变量，不能直接删除；必须作为情景变量保留，并在正文说明成立时的利润弹性、不成立时的降级路径，在终审 QA 中说明证伪方式。

## 输出口径

每份报告至少说明：

- 使用了哪些 skill 路径：公告、`$tushare`、`$wencai-query`、客户 capex、海关/出货、网页核验。
- 最新数据日期和报告期。
- 哪些结论来自公告事实，哪些来自推算或情景假设。
- 哪些来自券商、产业链或市场口径，为什么保留、如何验证。
- 哪些证据提高确信度，哪些证据降低确信度。

## 兜底原则

如果客户 capex、订单或海关数据缺失，不要直接写成强兑现。应把结论改成情景判断：若后续出现订单、排产、毛利率和现金流验证，主线升级；若长期缺失，逻辑降级为周期修复、补库涨价、普通放量或主题映射，并列出需要跟踪的数据。

## Valuation Handoff Order Override

When this file conflicts with the post-report handoff rule, the post-report rule wins.

Do not generate post-report handoffs during the research-writing path. After `final_report` exists, `skeptic_review` exists, and `final_report_gate.py` returns PASS, generate both lightweight handoffs:

```text
<prefix>_dcf_financial_model_handoff.md
<prefix>_peg_valuation_handoff.md
```

The DCF handoff contains only DCF driver admission, UFCF guardrails, and blocking gaps. The PEG handoff contains PEG factor treatment and must explain how each factor affects the PEG coefficient, not merely list positive or negative factors.
